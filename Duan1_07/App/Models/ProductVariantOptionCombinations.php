...
class ProductVariantOptionCombinations
{
    ...
    SELECT 
        ...
        pv.variant_id
    FROM 
        product_variant_option_combinations pvoc
    JOIN     
        product_variant_options pvo 
        ON pvo.product_variant_option_id = pvoc.product_variant_option_id
    JOIN 
        product_variants pv 
        ON pv.variant_id = pvo.variant_id
    WHERE 
        pv.product_id = ?
        ";
        ...
        try {
            ...
            // Đảm bảo chỉ đóng nếu biến không phải null
